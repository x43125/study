 ...
 * 分片键：code
 * 路由规则：
 *   1. code 为 null → t_order_item（兜底表）
 *   2. 老格式（ORD + 时间戳 + 随机数）→ 提取年月 → t_order_item_YYYYMM
 *   3. 雪花算法格式（新数据）→ 提取年月 → t_order_item_YYYYMM
 *   4. 未知格式 → t_order_item（兜底表）
 * 
 * 老数据示例：ORD17743643638409224
 *   - 前缀："ORD"（3位）
 *   - 时间戳：1774364363840（13位，毫秒）
 *   - 随机数：9224（4位）
 *   - 提取年月：202604 → t_order_item_202604
 * 
 * 新数据示例：1234567890123456789（雪花算法）
 *   - 提取年月：202604 → t_order_item_202604
 * 
 * 注意：与 OrderShardingAlgorithm 逻辑一致，确保 Order 和 OrderItem 作为绑定表在同一分片
 */
public class OrderItemShardingAlgorithm implements StandardShardingAlgorithm<Long> {
    ...
    public String doSharding(Collection<String> availableTargetNames, PreciseShardingValue<Long> shardingValue) {
        ...
        if (code == null) {
            System.out.println("code 为 null，路由到兜底表: t_order_item");
            return "t_order_item";
        }
        ...
        // 都无法识别，兜底
        System.out.println("code=" + code + " 无法识别格式，路由到兜底表: t_order_item");
        return "t_order_item";
    }
    ...
    private String routeByLegacy(Long code, Collection<String> availableTargetNames) {
        try {
            ...
            // 6. 构造目标表名
            String targetTable = "t_order_item_" + yearMonth;
    ...
    public void init(Properties properties) {
        System.out.println("==============================================");
        System.out.println("订单明细分片算法初始化成功");
        System.out.println("兼容老格式：ORD + 时间戳 + 随机数");
        System.out.println("兼容新格式：雪花算法ID");
        System.out.println("分片规则：按月分表（t_order_item_YYYYMM）");
        System.out.println("兜底表：t_order_item");
        System.out.println("==============================================");
    }
}